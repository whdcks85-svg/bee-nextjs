...
const DIMS = [
  { key: 'flexibility', label: '유연성', lo: '딱딱', hi: '유연' },
  { key: 'balance', label: '밸런스', lo: '헤드라이트', hi: '헤드헤비' },
  { key: 'weight', label: '체감무게', lo: '가볍다', hi: '무겁다' },
  { key: 'durability', label: '내구성', lo: '불만', hi: '만족' },
  { key: 'control', label: '컨트롤', lo: '어려움', hi: '쉬움' },
  { key: 'power', label: '파워', lo: '어려움', hi: '강함' },
]
...
export default function ReviewWriteClient() {
  ...
  const submit = async () => {
    if (body.length < 50) { alert('리뷰를 50자 이상 작성해주세요'); return }
    setLoading(true)
  ...
  if (done) return (
    <div style={{ padding: '40px', textAlign: 'center', fontFamily: 'sans-serif' }}>
      ...
      <p style={{ color: '#6B7280', marginBottom: '24px' }}>AI 검토 후 500P가 지급됩니다</p>
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '720px', margin: '0 auto', fontFamily: 'sans-serif', background: '#F7F6F3', minHeight: '100vh' }}>
      ...
      <div style={{ background: '#fff', border: '1.5px solid #D4D0CC', borderRadius: '10px', padding: '16px', marginBottom: '12px' }}>
        <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <p style={{ fontSize: '12px', color: '#6B7280', lineHeight: '1.5' }}>슬라이더를 움직이면 공식(점선)과 체감(주황) 비교가 보입니다</p>
            <p style={{ fontSize: '12px', color: '#6B7280', marginTop: '8px' }}>체감 평균 <b style={{ color: '#D85A30' }}>{avg}</b> / 5.0</p>
          </div>
        </div>
      </div>

      <div style={{ background: '#fff', border: '1.5px solid #D4D0CC', borderRadius: '10px', padding: '16px', marginBottom: '12px' }}>
        <h2 style={{ fontSize: '15px', fontWeight: '700', marginBottom: '4px' }}>스펙 체감 입력</h2>
        <p style={{ fontSize: '12px', color: '#6B7280', marginBottom: '14px' }}>왼쪽 끝 = 공식보다 부드러운 느낌</p>
        {DIMS.map((d, j) => (
      ...
      <div style={{ background: '#fff', border: '1.5px solid #D4D0CC', borderRadius: '10px', padding: '16px', marginBottom: '12px' }}>
        ...
        <h2 style={{ fontSize: '15px', fontWeight: '700', margin: '14px 0 8px' }}>종합 별점</h2>
